...
def load_imitations(data_folder):
    actions_path = sorted(glob.glob(os.path.join(data_folder, 'action_*.npy')))
    observations_path = sorted(glob.glob(os.path.join(data_folder, 'observation_*.npy')))
    action_id, observation_id, actions, observations = [], [], [], []

    for action_path in actions_path:
        action_id.append(int(os.path.basename(action_path).rstrip('.npy')[7:]))
        actions.append(np.load(action_path))

    for observation_path in observations_path:
        observation_id.append( int( os.path.basename(observation_path).rstrip('.npy')[12:] ) )
        observations.append(np.load(observation_path))
...
def save_imitations(data_folder, actions, observations):
    ...
    for action in actions:
        action_id += 1
        np.save((os.path.join(data_folder, 'action_00' + str(action_id ) + '.npy')), action)

    for observation in observations:
        observation_id += 1
        np.save((os.path.join(data_folder, 'observation_00' + str(observation_id) + '.npy')), observation)
